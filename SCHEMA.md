...
**Instrument version rule:** new submissions are stamped with the active `instruments` row for `pvua_v5_2` at creation time. Audit-session responses resolve the stored `(instrument_key, instrument_version)` first so historical submissions render/export against the version they used, not whichever version is active later. The database row metadata is authoritative; response builders override stale `content.en.instrument_version` values when a row was uploaded with mismatched embedded metadata. For legacy rows incorrectly stamped as `5.2`, the response builder compares stored response question keys against the stored-version instrument and active instrument, then uses the active instrument only when it covers more of the actual stored responses **and** reads every stored scale answer: each answered scale exists on that question, the answer shape matches the scale's `selection_mode` (a string for `single`, a list for `multiple`), and every chosen option key exists. Otherwise the stored version is kept. Answers cannot cross the Sociability boundary — single choice up to 5.31, multiple choice from 5.32 — so a single-choice submission is never read with a multi-choice version. Submit records the resolved `(instrument_key, instrument_version)` on the submission, so a submitted audit keeps reading against the version it was scored with after another version becomes active.
...
4. **Repair mis-stamped instrument versions.** Before removing compatibility, identify submitted rows with `instrument_version='5.2'` whose stored response question keys match the active/versioned 5.13 instrument better than the 5.2 instrument. Update those rows to the correct `instrument_version` in small batches after spot-checking rendered audit details and reports. The correct version is the one that was active when the row was submitted (the latest `instruments.created_at` before `submitted_at`), confirmed by every answered question key and scale option existing in it. The production cohort is 20 submitted rows from 2026-05-13..16: fifteen belong to 5.13 and five to 5.14, which score identically; every version from 5.10 to 5.31 reads them fully, and every version from 5.32 onward conflicts on Sociability.
