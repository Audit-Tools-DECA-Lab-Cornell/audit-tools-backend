...
def test_yee_mobile_release_policy_is_public(monkeypatch: pytest.MonkeyPatch) -> None:
	...
	assert body["android"]["latest_version"] == "0.10.0"
	assert body["android"]["minimum_supported_version"] == "0.10.0"
	assert body["android"]["update_url"].startswith("https://play.google.com/")
	assert body["ios"]["latest_version"] == "0.10.0"
	assert body["ios"]["minimum_supported_version"] == "0.10.0"
