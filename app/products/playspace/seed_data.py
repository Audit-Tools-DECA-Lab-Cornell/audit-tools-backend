...
AUDITOR_BLUEPRINTS: tuple[AuditorBlueprint, ...] = (
    ...
    AuditorBlueprint(
        "AKL-05", "Ethan Brooks", "Auckland", "25-34", "Man", "community youth worker"
    ),
    ...
    AuditorBlueprint(
        "TRG-01", "Isla Hughes", "Tauranga", "25-34", "Woman", "community facilitator"
    ),
...
def _build_auditor_contexts(*, reference_date: date) -> list[AuditorSeedContext]:
    ...
    for index, blueprint in enumerate(AUDITOR_BLUEPRINTS):
        ...
        account_id = (
            ...
            else _stable_uuid("playspace-account", blueprint.auditor_code)
        )
        profile_id = (
            fixed_ids[1]
            if fixed_ids is not None
            else _stable_uuid("playspace-profile", blueprint.auditor_code)
        )
    ...
    base_places = [
        PlaceSeedContext(
            place=Place(
                ...
                created_at=datetime.combine(
                    start_anchor + timedelta(days=1), time(9, 0), tzinfo=UTC
                ),
            ),
            project_context=project_context,
        ...
        PlaceSeedContext(
            place=Place(
                ...
                created_at=datetime.combine(
                    start_anchor + timedelta(days=2), time(9, 10), tzinfo=UTC
                ),
            ),
            project_context=project_context,
    ...
    base_places = [
        PlaceSeedContext(
            place=Place(
                ...
                created_at=datetime.combine(
                    start_anchor + timedelta(days=1), time(8, 45), tzinfo=UTC
                ),
            ),
            project_context=project_context,
    ...
    for index, project_context in enumerate(project_contexts):
        ...
        for place_index, place_context in enumerate(
            ...
            off_project_candidates = [
                auditor_context
                for auditor_context in auditor_contexts
                if auditor_context.profile.id
                not in {context.profile.id for context in project_auditors}
...
def _build_audits(
    *,
    ...
    audits: list[Audit] = []
    auditor_context_by_id = {
        auditor_context.profile.id: auditor_context for auditor_context in auditor_contexts
    }
    ...
    *,
    ...
    audits.append(
        _build_audit_record(
            ...
            randomizer=randomizer,
            status=draft_status,
            quality_bias=_bounded_bias(
                place_context.quality_bias + randomizer.uniform(-0.07, 0.04)
            ),
...
def _build_responses_json(
    *,
    ...
    if (
        target_completion_ratio > 0
        and target_answered_questions == 0
        and total_visible_questions > 0
    ):
...
def _build_section_note(
    *,
    ...
    if is_complete:
        return (
            f"Captured final observations for {humanized_section} at {place_name} with emphasis on "
            f"{focus_label}."
        )
...
def _progress_percent(*, progress: object) -> float:
    ...
    if not hasattr(progress, "total_visible_questions") or not hasattr(
    ...
    if not isinstance(total_visible_questions, int) or not isinstance(
...
def _build_place_description(
    *,
    ...
    return (
        f"{tone} {place_type} conditions with emphasis on {focus_terms[0]}, "
        f"{focus_terms[1]}, and {focus_terms[2]}."
    )
...
def _build_audit_code(
    *,
    ...
    slot_segment = (
        "".join(character for character in slot_key.upper() if character.isalnum())[:4] or "RUN"
    )
...
def _email_from_name(full_name: str) -> str:
    ...
    slug = "".join(
        character.lower() if character.isalnum() else "." for character in full_name.strip()
    )
