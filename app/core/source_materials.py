...
def build_yee_source_metadata() -> dict[str, object]:
	...
	instructions_path = (
		repo_root / "yee" / "instructions" / "Instructions for viewing YEE Audit Tool questions and scoring.txt"
	)
	...
	instructions_lines = [
		_clean_text(line) for line in instructions_path.read_text(encoding="utf-8").splitlines() if _clean_text(line)
	]

	return {
		"instrument_key": str(survey_entry.get("SurveyID", "yee_seed_instrument")),
		"instrument_name": _clean_text(str(survey_entry.get("SurveyName", "Youth Enabling Environments Audit Tool"))),
