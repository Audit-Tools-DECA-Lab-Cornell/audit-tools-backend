...
def _link_auditor_profiles_to_users() -> None:
    ...
    linked_user_ids = set(
        bind.execute(
            sa.select(auditor_profiles_table.c.user_id).where(
                auditor_profiles_table.c.user_id.is_not(None)
            )
        ).scalars()
    )
    ...
    for row in profile_rows:
        ...
        bind.execute(
            auditor_profiles_table.update()
            .where(auditor_profiles_table.c.id == row["id"])
