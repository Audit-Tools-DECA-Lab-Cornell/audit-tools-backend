"""Add YEE bug-reporting and known-issues tables.

Creates the internal bug-reporting workflow tables in the YEE database:
...
These are the SAME shared-core tables Playspace created in ``ps_0006``; both
product databases now hold them physically (they were removed from
``PLAYSPACE_ONLY_TABLE_NAMES``). The one product-specific difference is the audit
reference on ``bug_reports``:

* ``yee_submission_id`` carries a real FK to ``yee_audit_submissions`` (present in
  the YEE database).
* ``playspace_submission_id`` exists as a plain nullable UUID column with NO FK,
  because ``playspace_submissions`` does not exist in the YEE database.
...
def upgrade() -> None:
	...
	if not _has_table("bug_reports"):
		op.create_table(
			...
			# No FK: playspace_submissions does not exist in the YEE database.
			sa.Column("playspace_submission_id", sa.UUID(), nullable=True),
			sa.Column("yee_submission_id", sa.UUID(), nullable=True),
			...
			sa.ForeignKeyConstraint(
				["yee_submission_id"],
				["yee_audit_submissions.id"],
				name=op.f("fk_bug_reports_yee_submission_id_yee_audit_submissions"),
				ondelete="SET NULL",
			),
