"""Initial shared-core schema.

Creates the tables shared by BOTH product databases (YEE and Playspace). It is
the common base/root of the migration tree. Product-specific tables are created
by the per-product branches that descend from this revision:

    - ``playspace`` branch → ``ps_0001`` and later (playspace_* tables)
    - ``yee`` branch       → ``yee_0001`` and later (yee_* tables)

Run a single product up to its branch head::
...
(through ``20260514_0010``) must be **stamped**, not upgraded. See
``docs/deployment.md`` for the stamp/cutover procedure.
